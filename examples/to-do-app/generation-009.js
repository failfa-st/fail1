...
async function output() {
  ...
  Object.entries(groupedTasks).forEach(([priority, tasks]) => {
    ...
    tasks.sort((a, b) => {
      ...
      console.log(`\t${index + 1}. ${task.task}${task.done ? " (done)" : ""}${task.reminder ? ` [${task.reminder}]` : ""}`);
    });
  });
}
  ...
  rl.question("Enter task: ", async (answer) => {
    rl.question("Enter priority (optional): ", async (priority) => {
      rl.question("Enter reminder (optional): ", async (reminder) => {
        tasks.push({ task: answer, done: false, priority: priority || null, reminder: reminder || null });
  ...
  rl.question("Enter keyword: ", async (answer) => {
    ...
    if (filteredTasks.length === 0) {
      ...
      filteredTasks.forEach((task, index) => {
        console.log(`${index + 1}. ${task.task}${task.done ? " (done)" : ""}${task.priority ? ` [${task.priority}]` : ""}${task.reminder ? ` [${task.reminder}]` : ""}`);
      });
    }
    rl.close();
  });
